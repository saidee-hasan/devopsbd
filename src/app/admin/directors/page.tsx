...
const initialDirectors: Director[] = [
  ...
  },
  {
    ...
    bio: "7+ years leading large‑scale engineering initiatives with a focus on delivery excellence.",
  },
];
...
export default function AdminDirectorsPage() {
  ...
  const [form, setForm] = useState<Director>({ name: "", title: "", bio: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    // In a real app you would POST to an API. For now we just update local state.
    setDirectors([...directors, form]);
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-200">Engineering Directors</h1>
        <button
        ...
        {directors.map((d, idx) => (
          <div key={idx} className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-5">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-lg font-semibold text-slate-200">{d.name}</h2>
            </div>
            <p className="text-sm font-medium text-emerald-300 mb-1">{d.title}</p>
            <p className="text-sm text-slate-400">{d.bio}</p>
          </div>
        ))}
        {directors.length === 0 && (
          <p className="text-base text-slate-500">No directors added yet.</p>
        )}
      </div>
    </div>
  );
